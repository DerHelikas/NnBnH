    ...
    class DB_PostgreSQLconnectionClass : DB_CommonDBConnectionClass
    {
        ...
        public override List<KanaGroupedLine> LondAllKanaFromDB(string KanaTableName, out Exception ProcessExcetion)
        {
            ...
            try
            {
                ...
                var SortedKana = KanaSeq.GroupBy(count => count.Pronunciation.Length);
                ...
                KanaTable.Add(new KanaGroupedLine("k*",
                    SortedKana.ElementAtOrDefault(1)?.Where(kana => kana.Pronunciation.StartsWith("k")).ToList()));

                // ga gi gu ge go
                KanaTable.Add(new KanaGroupedLine("g*",
                    SortedKana.ElementAtOrDefault(1)?.Where(kana => kana.Pronunciation.StartsWith("g")).ToList()));


                // sa si su se so
                KanaTable.Add(new KanaGroupedLine("s*",
                    SortedKana.ElementAtOrDefault(1)?.Where(kana => kana.Pronunciation.StartsWith("s")).ToList()));

                // za zi zu ze zo
                KanaTable.Add(new KanaGroupedLine("z*",
                    SortedKana.ElementAtOrDefault(1)?.Where(kana => kana.Pronunciation.StartsWith("z")).ToList()));


                // ta-to
                KanaTable.Add(new KanaGroupedLine("t*",
                    SortedKana.ElementAtOrDefault(1)?.Where(kana => kana.Pronunciation.StartsWith("t")).ToList()));
                // da-do
                KanaTable.Add(new KanaGroupedLine("d*",
                    SortedKana.ElementAtOrDefault(1)?.Where(kana => kana.Pronunciation.StartsWith("d")).ToList()));


                // ra-ro
                KanaTable.Add(new KanaGroupedLine("r*",
                    SortedKana.ElementAtOrDefault(1)?.Where(kana => kana.Pronunciation.StartsWith("r")).ToList()));


                // na-no
                KanaTable.Add(new KanaGroupedLine("n*",
                    SortedKana.ElementAtOrDefault(1)?.Where(kana => kana.Pronunciation.StartsWith("n")).ToList()));


                // ma-mo
                KanaTable.Add(new KanaGroupedLine("m*",
                    SortedKana.ElementAtOrDefault(1)?.Where(kana => kana.Pronunciation.StartsWith("m")).ToList()));
                ...
                KanaTable.Add(new KanaGroupedLine("h*",
                    SortedKana.ElementAtOrDefault(1)?.Where(kana => kana.Pronunciation.StartsWith("h")).ToList()));

                // pa-po
                KanaTable.Add(new KanaGroupedLine("p*",
                    SortedKana.ElementAtOrDefault(1)?.Where(kana => kana.Pronunciation.StartsWith("p")).ToList()));

                // ba-bo
                KanaTable.Add(new KanaGroupedLine("b*",
                    SortedKana.ElementAtOrDefault(1)?.Where(kana => kana.Pronunciation.StartsWith("b")).ToList()));

                // wa-wo
                KanaTable.Add(new KanaGroupedLine("w*",
                    SortedKana.ElementAtOrDefault(1)?.Where(kana => kana.Pronunciation.StartsWith("w")).ToList()));

                // y*
                KanaTable.Add(new KanaGroupedLine("y*",
                    SortedKana.ElementAtOrDefault(1)?.Where(kana => kana.Pronunciation.StartsWith("y")).ToList()));
